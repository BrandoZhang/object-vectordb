...
from object_vectordb import SearchResult, rrf_merge
...
def test_rrf_merge_respects_limit():
    ...
    fused = rrf_merge(a, b, k=60, limit=2)
    assert len(fused) == 2
...
def test_rrf_merge_k_parameter_affects_scores():
    ...
    small_k = rrf_merge(a, b, k=1)
    large_k = rrf_merge(a, b, k=1000)
