...
from object_store import ObjectStore, SearchResult
...
def test_rrf_merge_basic_fusion():
    ...
    fused = ObjectStore.rrf_merge(a, b, k=60)
...
def test_rrf_merge_respects_limit():
    ...
    fused = ObjectStore.rrf_merge(a, b, k=60, limit=2)
    assert len(fused) == 2
...
def test_rrf_merge_k_parameter_affects_scores():
    ...
    small_k = ObjectStore.rrf_merge(a, b, k=1)
    large_k = ObjectStore.rrf_merge(a, b, k=1000)
