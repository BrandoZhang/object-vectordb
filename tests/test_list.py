...
def test_list_with_where(store):
    _seed(store)
    out = store.list_objects(where="n >= 3")
...
def test_list_with_select_trims_properties(store):
    _seed(store)
    out = store.list_objects(where="n = 2", select=["n"])
    assert len(out) == 1
...
def test_list_with_limit(store):
    _seed(store)
    out = store.list_objects(limit=2)
    assert len(out) == 2


def test_list_with_offset(store):
    _seed(store)
    out_all = store.list_objects(limit=5)
    out_offset = store.list_objects(limit=5, offset=2)
